<template>
  <div id="hy-swiper">
    <div class="swiper" ref="swiper" @touchstart="touchStart" @touchmove="touchMove"
         @touched="touchEnd">
      <slot></slot>
    </div>
    <slot name="indicator">
    ...
    <div class="indicator">
      <slot name="indicator" v-if="showIndicator && slideCount > 1" >
        <div v-for="index in slideCount" class="indi-item" :class="{active:currentIndex-1 === index}" :key='index'></div>
      </slot>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    props: {
      ...
      //手指向左拉动多少比例 轮播图片显示下一张
      moveRatio: {
        type: Number,
        default: 0.25
      },
      //显示指示器：轮播图下面四个小点 表示轮播到第几张图
      showIndicator: {
        type: Boolean,
        default: true
      }
